...
def process_first_two_frames(img1: np.ndarray,
                             img2: np.ndarray,
                             K: np.ndarray,
                             timestamp1: float = 0.0,
                             timestamp2: float = 0.0) -> tuple[VOState, LandmarkDatabase, bool]:
    """
...
def process_frame(img: np.ndarray,
                  ...
                  K: np.ndarray,
                  timestamp: float,
                  next_track_id: int) -> tuple[VOState, LandmarkDatabase, bool, int]:
    """
...
def run_vo_pipeline(images: list[np.ndarray],
                    timestamps: list[float],
                    K: np.ndarray) -> tuple[list[VOState], LandmarkDatabase]:
    """
    ...
    vo_state, landmark_db, success = process_first_two_frames(
        images[0], images[1], K, timestamps[0], timestamps[1]
    )
    ...
    for i in range(2, len(images)):
        ...
        vo_state, landmark_db, success, next_track_id = process_frame(
            images[i], vo_state, vo_state_prev, landmark_db, 
            K, timestamps[i], next_track_id
        )
