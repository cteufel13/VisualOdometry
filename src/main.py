...
def main():
    ...
    own_dataset_path = "../data/own_dataset"
    ...
    if ds == 0:
        ...
        K = np.loadtxt(io.StringIO(cleaned_text), dtype=np.float64)
    ...
    if ds == 0:
        img0 = cv2.imread(
            os.path.join(kitti_path, "05", "image_0", f"{bootstrap_frames[0]:06d}.png"),
            cv2.IMREAD_GRAYSCALE,
        )
        img1 = cv2.imread(
            os.path.join(kitti_path, "05", "image_0", f"{bootstrap_frames[1]:06d}.png"),
            cv2.IMREAD_GRAYSCALE,
        )
    elif ds == 1:
        ...
        img0 = cv2.imread(
            os.path.join(parking_path, "images", f"img_{bootstrap_frames[0]:05d}.png"),
            cv2.IMREAD_GRAYSCALE,
        )
        img1 = cv2.imread(
            os.path.join(parking_path, "images", f"img_{bootstrap_frames[1]:05d}.png"),
            cv2.IMREAD_GRAYSCALE,
        )
    elif ds == 3:
        # Load images from own dataset
        img0 = cv2.imread(
            os.path.join(own_dataset_path, f"{bootstrap_frames[0]:06d}.png"),
            cv2.IMREAD_GRAYSCALE,
        )
        img1 = cv2.imread(
            os.path.join(own_dataset_path, f"{bootstrap_frames[1]:06d}.png"),
            cv2.IMREAD_GRAYSCALE,
        )
    else:
        raise ValueError("Invalid dataset index")
