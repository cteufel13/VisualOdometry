...
def update_landmark_database(vo_state: VOState,
...
def should_create_keyframe(vo_state_curr: VOState,
                          vo_state_prev: VOState,
                          num_matches: int,
                          num_landmarks: int) -> bool:
    """
...
def add_new_landmarks(landmark_db: LandmarkDatabase,
                      new_landmarks_3d: np.ndarray,
                      new_descriptors: np.ndarray,
                      next_track_id: int) -> tuple[LandmarkDatabase, int]:
    """
...
def filter_landmarks(landmark_db: LandmarkDatabase,
                     min_observations: int = 3,
                     vo_state: VOState | None = None,
                     K: np.ndarray | None = None) -> LandmarkDatabase:
    """
...
def compute_triangulation_quality(points_3d: np.ndarray,
...
def update_observation_counts(landmark_db: LandmarkDatabase,
                              matched_indices: np.ndarray,
                              inlier_mask: np.ndarray) -> LandmarkDatabase:
    """
